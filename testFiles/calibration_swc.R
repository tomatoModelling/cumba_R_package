...
par$DepletionFraction$value<-20

#TODO: calibrate
par$RootIncrease$value<-.5
...
out<-do.call(rbind,outputs_list) |> 
  ...
  mutate(swc_sim=(wc1*30+wc2*(rootState-3)*10+wc3*(60-rootState)*10)/600) |> 
...
ggplot(out ,aes(x=doy)) + 
  ...
  geom_line(aes(y=swc_sim),col='gold2',linewidth=1.2)+
